...
use crate::torrent::{in_list, DownloadStatus};
...
pub async fn status_of(client: &Client, id: i32) -> Option<String> {
    match client
...
/// Read a row's status, hard-failing on any query error (incl. `RowNotFound`)
/// — for the site where a DB blip must not mask a cancel.
...
pub async fn status_if_changed(pool: &Pool, id: i32, expected: &str) -> Option<String> {
    ...
    match status_of(&client, id).await {
        Some(status) => (status != expected).then_some(status),
        None => None,
    }
}
...
/// on a query blip). Takes an already-checked-out `&Client` so the caller can
/// pair the read with a follow-up write on the same connection.
pub async fn status_no_longer_downloading(client: &Client, id: i32) -> Option<String> {
    status_of(client, id)
        .await
...
pub async fn claim_direct(pool: &Pool, id: i32, part: &str) -> Result<u64> {
    ...
    client
        .execute(
            &format!(
                "UPDATE downloads SET status = {down}, file_path = $2, updated_at = now() \
                 WHERE id = $1 AND status = {queued}",
                down = DownloadStatus::Downloading.as_str(),
...
pub async fn mark_error(pool: &Pool, id: i32, msg: &str) {
    if let Ok(client) = pool.get().await {
        let _ = client
            .execute(
                &format!(
                    "UPDATE downloads SET status = {err}, error = $2, updated_at = now() \
                     WHERE id = $1 AND status NOT IN ({terminal})",
                    err = DownloadStatus::Error.as_str(),
                    terminal = in_list([
...
pub async fn mark_fatal_error(pool: &Pool, id: i32, msg: &str) -> Result<()> {
    ...
    let _ = client
        .execute(
            &format!(
                "UPDATE downloads SET status = {err}, error = $2, updated_at = now() WHERE id = $1",
                err = DownloadStatus::Error.as_str()
            ),
            &[&id, &msg],
        )
...
pub async fn requeue_stale_errors(pool: &Pool, ids: &[i32]) -> Result<u64> {
    ...
    client
        .execute(
            &format!(
                "UPDATE downloads SET status = {queued}, error = NULL, updated_at = now() \
                 WHERE id = ANY($1) AND status = {err}",
                queued = DownloadStatus::Queued.as_str(),
                err = DownloadStatus::Error.as_str()
            ),
            &[&ids],
...
pub async fn finalize_direct(pool: &Pool, id: i32, dst: &str) -> Result<u64> {
    ...
    client
        .execute(
            &format!(
                "UPDATE downloads SET status = {done}, progress = 1.0, file_path = $2, \
                 updated_at = now() WHERE id = $1 AND status = {down}",
                done = DownloadStatus::Complete.as_str(),
                down = DownloadStatus::Downloading.as_str()
            ),
            &[&id, &dst],
        )
...
/// their seed stats (`$3 = 'seeding'`), everything else clears them. Returns
/// the row count (0 = a cancel raced the install; the caller discards).
///
/// `new_status` is the settled status (`seeding` when `keep_completed` + qB
/// present, else `complete`); the seed stats (`ratio`, `up_speed_bps`,
/// `num_seeds`) are `Some` only for the seeding arm.
        ...
        .execute(
            &format!(
                "UPDATE downloads SET \
                 status = CASE WHEN $3 = {seed} THEN {seed} ELSE {done} END, \
                 progress = 1.0, file_path = $2, \
                 ratio = CASE WHEN $3 = {seed} THEN $4 ELSE NULL END, \
                 up_speed_bps = CASE WHEN $3 = {seed} THEN $5 ELSE NULL END, \
                 num_seeds = CASE WHEN $3 = {seed} THEN $6 ELSE NULL END, \
                 error = NULL, updated_at = now() \
                 WHERE id = $1 AND status IN ({in_list})",
                seed = DownloadStatus::Seeding.as_str(),
                done = DownloadStatus::Complete.as_str(),
                in_list = in_list(completion_guard_statuses())
            ),
            &[
                &id,
                &dst,
...
pub async fn mark_downloading(pool: &Pool, id: i32) -> Result<()> {
    ...
    let _ = client
        .execute(
            &format!(
                "UPDATE downloads SET status = {down}, updated_at = now() WHERE id = $1",
                down = DownloadStatus::Downloading.as_str()
            ),
            &[&id],
        )
...
pub async fn bind_hash(pool: &Pool, id: i32, hash: &str) -> Result<()> {
    ...
    let _ = client
        .execute(
            "UPDATE downloads SET hash = $2, updated_at = now() WHERE id = $1",
            &[&id, &hash],
        )
        .await;
    Ok(())
...
pub async fn drain_cancelled_hashes(pool: &Pool) -> Result<u64> {
    ...
    client
        .execute(
            &format!(
                "UPDATE downloads SET hash = NULL WHERE status = {cancelled} AND hash IS NOT NULL",
                cancelled = DownloadStatus::Cancelled.as_str()
            ),
            &[],
        )
        .await
    ...
    let _ = match error {
        Some(msg) => {
            client
                .execute(
                    &format!(
                        "UPDATE downloads SET status = {done}, error = $2, updated_at = now() \
                     WHERE id = $1 AND status = {seed}",
                        done = DownloadStatus::Complete.as_str(),
                        seed = DownloadStatus::Seeding.as_str()
                    ),
                    &[&id, &msg],
        ...
        None => {
            client
                .execute(
                    &format!(
                        "UPDATE downloads SET status = {done}, updated_at = now() \
                     WHERE id = $1 AND status = {seed}",
                        done = DownloadStatus::Complete.as_str(),
                        seed = DownloadStatus::Seeding.as_str()
                    ),
...
pub async fn refresh_seeding_stats(
    ...
    let _ = client
        .execute(
            &format!(
                "UPDATE downloads SET ratio = $2, up_speed_bps = $3, num_seeds = $4, \
                 speed_bps = $5, updated_at = now() WHERE id = $1 AND status = {seed}",
                seed = DownloadStatus::Seeding.as_str()
            ),
            &[&id, &ratio, &up_speed_bps, &num_seeds, &speed_bps],
        )
        .await;
    Ok(())
}
...
pub async fn refresh_progress(pool: &Pool, id: i32, progress: f32, speed_bps: i64) -> Result<()> {
    ...
    let _ = client
        .execute(
            "UPDATE downloads SET progress = $2, speed_bps = $3, updated_at = now() WHERE id = $1",
            &[&id, &progress, &speed_bps],
        )
        .await;
    Ok(())
}
...
pub async fn retry_interrupted_directs(pool: &Pool) -> Result<u64> {
    ...
    client
        .execute(
            &format!(
                "UPDATE downloads SET status = {queued}, file_path = NULL, error = NULL, updated_at = now() \
                 WHERE status = {down} AND {pred} LIKE '%.zim'",
                queued = DownloadStatus::Queued.as_str(),
                down = DownloadStatus::Downloading.as_str(),
                pred = crate::db::downloads::ZIM_URL_PREDICATE
            ),
            &[],
        )
        .await
