...
pub async fn auth_middleware(
    ...
    let ip = request
        ...
        .map(|ci| ci.0.ip());
    // WI-14: when `general.trusted_proxy_cidrs` is set and the source IP is
    // within a trusted CIDR, walk `X-Forwarded-For` right-to-left skipping
    // trusted hops to find the real client IP for the lockout key.
    let ip = ip.map(|ip| {
        let cidrs_raw = state
            .settings
            .get_typed::<String>(KEY_GENERAL_TRUSTED_PROXY_CIDRS)
            .unwrap_or_default();
        if cidrs_raw.trim().is_empty() || !cidrs_contains(&cidrs_raw, &ip) {
            return ip;
        }
        request
            .headers()
            ...
            .and_then(|xff| {
                xff.split(',')
                    ...
                    .find(|candidate| !cidrs_contains(&cidrs_raw, candidate))
            })
            .unwrap_or(ip)
    });
...
mod tests {
    ...
    #[test]
    fn xff_walk_rightmost_non_trusted() {
        // 3 proxy hops: XFF = "1.2.3.4, 10.0.0.1, 10.0.0.2"
        // Trusted: 10.0.0.0/8 → skip 10.0.0.2, 10.0.0.1 → client is 1.2.3.4
        let cidrs = "10.0.0.0/8";
        let xff = "1.2.3.4, 10.0.0.1, 10.0.0.2";
        let client = xff
            .split(',')
            .rev()
            .map(|s| s.trim())
            ...
            .unwrap();
        assert_eq!(client, ipp("1.2.3.4"));
    }

    #[test]
    fn xff_attacker_spoofed_leftmost() {
        // Attacker puts their own IP at the leftmost position:
        // XFF = "6.6.6.6, 10.0.0.1"  (10.0.0.1 is the proxy)
        // Walking right-to-left: 10.0.0.1 is trusted → skip → next is 6.6.6.6
        // The attacker can only spoof IPs that are ALSO in the trusted range
        // (since we stop at the first non-trusted from the right).
        let cidrs = "10.0.0.0/8";
        let xff = "6.6.6.6, 10.0.0.1";
        let client = xff
            .split(',')
            .rev()
            .map(|s| s.trim())
            ...
            .find(|c| !cidrs_contains(cidrs, c))
            .unwrap();
        assert_eq!(client, ipp("6.6.6.6"));
    }

    #[test]
    fn xff_no_header_falls_back_to_connect_ip() {
        // No XFF header → the walk returns None → the caller uses the
        // ConnectInfo IP (verified by the middleware code path).
        let cidrs = "10.0.0.0/8";
        let xff: Option<&str> = None;
        let client = xff.and_then(|s| {
            s.split(',')
                .rev()
                .map(|s| s.trim())
                .filter_map(|s| s.parse::<std::net::IpAddr>().ok())
    ...
    #[test]
    fn xff_all_trusted_falls_back_to_connect_ip() {
        // Every XFF entry is trusted → the walk returns None → caller uses
        // the ConnectInfo IP.
        let cidrs = "10.0.0.0/8";
