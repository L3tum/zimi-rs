...
impl SettingsCache {
    ...
    //
    // The plain accessors fall back to their `SETTING_DEFS` seed default via
    // `unwrap_or` — `reload()` seeds every key, so the fallback only fires if
    // a seed is ever missing or mistyped (a test pins the seed set — see
    // `default_settings_seeds_all_typed_accessor_keys` in `defs.rs`). The
    // accessors with extra logic (clamps, the `Option` pass-through, the
    // documented read-auth flag) use the `default_value`-based form instead.
    ...
    /// Typed read accessor for this setting (`downloads.allow_private_networks`).
    ...
    pub fn downloads_allow_private_networks(&self) -> bool {
        self.get_typed(KEY_DOWNLOADS_ALLOW_PRIVATE_NETWORKS)
            .unwrap_or(false)
    }
    ...
    /// Every key is seeded by `reload()`; the `unwrap_or` is an honest
    /// fallback to the `SETTING_DEFS` seed default if a seed is ever missing.
    pub fn torrent_max_active(&self) -> i32 {
        self.get_typed(KEY_TORRENT_MAX_ACTIVE).unwrap_or(4)
    }

    /// Typed read accessor for this setting (`embedding.enabled`).
    /// Every key is seeded by `reload()`; the `unwrap_or` is an honest
    /// fallback to the `SETTING_DEFS` seed default if a seed is ever missing.
    pub fn embedding_enabled(&self) -> bool {
        self.get_typed(KEY_EMBEDDING_ENABLED).unwrap_or(false)
    }

    /// Typed read accessor for this setting (`access.mode`).
    /// Every key is seeded by `reload()`; the `unwrap_or` is an honest
    /// fallback to the `SETTING_DEFS` seed default if a seed is ever missing.
    pub fn access_mode(&self) -> String {
        self.get_typed(KEY_ACCESS_MODE)
            .unwrap_or(ACCESS_MODE_OPEN.to_string())
    }
